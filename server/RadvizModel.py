...
class RadvizModel(DomainModel):
    ...
    def getRadvizPoints(self, session, filterByTerm, typeRadViz, nroCluster, removeKeywords):
        ...
        if typeRadViz == "1":
            ...
            labels_t = unique_labels_train
            label_by_classes = self.getNumberClasses(labels_t)
            #print "label_by_classes", label_by_classes
            [features_uniques, new_X_sum,new_X_sum_copy, classes_labels, newLabels, newUrls, newTitles, newSnippets,newImageUrls]  = self.getAllSamples_inClasses_RemoveCommonFeatures(label_by_classes, unique_labels_train, urls_train,titles_train, snippets_train,  image_urls_train,  data_train, max_features_in_cluster)
            #cluster_labels = classes_labels
            features = features_uniques
