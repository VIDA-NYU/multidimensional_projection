...
function RadvizUtils(data){
    if (!data){
        throw "Error. RadvizUtils requires a dataset to work with."
    }
    ...
    this.translate = 1;
    this.scale = 10;
    //this.radvisWC = new RadvizUtilsWordCloud();
}

RadvizUtils.prototype.updateSigmoid = function (translate,scale) {
...
RadvizUtils.prototype.asFactor = function(d)
{
...
RadvizUtils.prototype.setHideElements = function (selection) {
    for (var id in selection){
        this.hidden[selection[id]] = true;
    }
};

RadvizUtils.prototype.setHideUnselected = function (selection) {
...
RadvizUtils.prototype.unhideAll = function(){
    this.hidden = [];
};


/*RadvizUtils.prototype.setSelected = function (selection) {
    this.selected = [];
    var _this = this;
    $("#selectionList").html('');
    for (var id in selection){
...
//Sonia
/*RadvizUtils.prototype.setSelectedWordCloud = function(selection){
  $('#wordCloudR').html('');
  var frequency_list = [];
  var hashmapTerms = {};
  ...
  RadvizUtilsWordCloud.prototype.wordCloud(frequency_list);
}*/



RadvizUtils.prototype.setColorsColumnId = function (columnId) {
    if (columnId == "density"){
        this.colorIsDensity = true;
        this.isContinuous = true;
        this.colors = numeric.rep([this.myData[this.dimNames[0]].length], 0);
...
RadvizUtils.prototype.normalizeData = function(){
...
RadvizUtils.prototype.sigmoid = function(x){
...
RadvizUtils.prototype.compute_yi = function(){
...
RadvizUtils.prototype.setAnchors = function(anchors) {
...
RadvizUtils.prototype.updateAnchors = function(anchors) {
    this.anchorAngles = [];
    this.weights = [];
    var _this = this;
    anchors.forEach(function(a){
...
RadvizUtils.prototype.anglesToXY = function(){ //transform this.anchorAngles to position matrix[[x,y]] and returns
    var initPoint = [1,0];
    var anchorMatrix = [];
...
RadvizUtils.prototype.computeProjection = function() {
...
RadvizUtils.prototype.selectColumns = function(columns, groupColumns) {
    this.mat_t = [];
    if (columns.length == 0){
        this.mat_t = [[]];
        this.matrix = [[]];
...
RadvizUtils.prototype.normalizeGroups = function(groupColumns){
...
RadvizUtils.prototype.getDimensionNames = function () {
    return Object.keys(this.myData);
};
RadvizUtils.prototype.getSizeData = function () {
  var size_data = 0;
  for(var i in this.myData){
    size_data++;
  }
